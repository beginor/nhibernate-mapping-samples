  ...
  class Program {
    static void Main(string[] args) {
      ...
      foreach (Contact contact in session.CreateQuery("from Contact").List<Contact>()) {
        Console.WriteLine(string.Format("{0} {1} '{2}'|  Person: '{3}' {4}", contact.Key.FirstName, contact.Key.LastName, contact.Phone, contact.Person.BirthDay.ToString(), contact.Person.Address));
      }
      Console.Read();
    }
  }
