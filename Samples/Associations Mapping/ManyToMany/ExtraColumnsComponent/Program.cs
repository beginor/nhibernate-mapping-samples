  ...
  class Program {
    static void Main(string[] args) {
      ...
      foreach (Company company in session.CreateQuery("from Company").List<Company>()) {
        ...
        foreach (CompanyFactoryType сompanyFactory in company.Factories)
          Console.WriteLine(string.Format("                   Factory: '{0}'.  {1} ", сompanyFactory.Factories.FactoryName, сompanyFactory.Description));
      ...
      foreach (Factory factory in session.CreateQuery("from Factory").List<Factory>()) {
        ...
        foreach (CompanyFactoryType сompanyFactory in factory.Companies)
          Console.WriteLine(string.Format("                   Company: '{0}'. {1}", сompanyFactory.Companies.CompanyName, сompanyFactory.Description));
      }
      Console.Read();
    }
  }
