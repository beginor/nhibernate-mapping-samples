...
namespace TPC {
  class Program {
    static void Main(string[] args) {
      ...
      Console.WriteLine("------------------------------------------------------");
      foreach (Dog dog in session.CreateQuery("from Dog").List<Dog>())
        Console.WriteLine(string.Format("Class:'{0}' FoodClassification:'{1}' Breed:'{2}'", dog.GetType().Name, dog.FoodClassification, dog.Breed));
