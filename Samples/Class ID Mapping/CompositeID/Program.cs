  ...
  class Program {
    static void Main(string[] args) {
      ...
      foreach (DepartmentPhone DepartmentPhone in result) {
        Console.WriteLine(string.Format("PK({0}-{1}): '{2}'  {3}  {4}", DepartmentPhone.DeptID, DepartmentPhone.PhoneID, DepartmentPhone.PhoneNumber, DepartmentPhone.DeptName, DepartmentPhone.Location));
      }
